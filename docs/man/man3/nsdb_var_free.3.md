...
numstore_var_len, numstore_var_free - inspect and release a numstore variable handle
...
*void*
**numstore_var_free**(*numstore_t \*db*, *numstore_var_t \*var*);

# DESCRIPTION

*numstore_var_t* handles are returned by variable-lookup operations such
as a **GET** query executed through **numstore_fexecute**(3) (see
...
: Releases *var* and all memory associated with it. *db* must be the
  ...
  **if exists** returns a *NULL* *var* on success when the variable
  ...
  themselves before calling **numstore_var_free**().

# RETURN VALUE

**numstore_var_len**() returns the element count of *var*. It has no
failure mode of its own; *var* is assumed to be a valid, non-NULL
handle.

**numstore_var_free**() returns nothing.

# NOTES

Every non-NULL *numstore_var_t* handle obtained from the API should
eventually be passed to **numstore_var_free**() to avoid leaking the
memory backing it.

# EXAMPLES

    numstore_var_t *var;
    if (numstore_fexecute (db, NULL, "get foo", &var) == 0) {
        b_size n = numstore_var_len (var);
        printf ("foo has %zu elements\n", (size_t) n);
        numstore_var_free (db, var);
    }

Handling a possibly-absent variable:

    numstore_var_t *var;
    numstore_fexecute (db, NULL, "get if exists foo", &var);
    if (var != NULL) {
        /* ... use var ... */
    }
    numstore_var_free (db, var);   /* safe even if var is NULL */

# SEE ALSO

**numstore**(1), **numstore_fexecute**(3)
