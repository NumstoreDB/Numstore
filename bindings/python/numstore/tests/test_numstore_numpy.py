...
def test_primitive(np_str, expected):
    assert numpy_to_numstore(np.dtype(np_str)) == expected
...
def test_accepts_python_type():
    assert numpy_to_numstore(np.int32) == "i32"


def test_byte_order_ignored():
    # Non-native byte-order maps to the same numstore primitive (platform endian).
...
def test_simple_struct():
    ...
    assert numpy_to_numstore(d) == "struct { foo i32, bar f64 }"
...
def test_simple_union():
    ...
    assert numpy_to_numstore(d) == "union { a i32, b f32 }"
...
def test_multidim_array():
    assert numpy_to_numstore(np.dtype(("f8", (20, 30)))) == "[20][30] f64"


def test_array_of_struct():
    d = np.dtype(([("x", "i4"), ("y", "f4")], (5,)))
    assert numpy_to_numstore(d) == "[5] struct { x i32, y f32 }"
...
def test_unsupported(np_str):
    with pytest.raises(ValueError):
        numpy_to_numstore(np.dtype(np_str))
