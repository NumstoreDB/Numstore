...
err_t ns_write_var_page (struct ns_write_var_page_params *params, error *e) {
  ...
  void              *dtype_serialized = NULL;
  u16                tlen             = type_get_serial_size (params->var->dtype);
  {
    chunk_alloc_create_default (&temp);
    dtype_serialized = chunk_malloc (&temp, tlen, 1, e);
    if (dtype_serialized == NULL) { return error_trace (e); }
  ...
  if (page_h_pgno (params->vp) != start) {
    ...
    if (pgr_get (params->vp, PG_VAR_PAGE, pg, params->p, e)) { goto theend; }
  }
