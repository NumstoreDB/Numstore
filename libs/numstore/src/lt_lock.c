...
u32 lt_lock_key (const struct lt_lock lock) {
...
void i_print_lt_lock (const int log_level, const struct lt_lock l) {
  switch (l.type) {
    case LOCK_DB: {
...
bool get_parent (struct lt_lock *parent, const struct lt_lock lock) {
  switch (lock.type) {
    case LOCK_DB: {
      return false;
    }
