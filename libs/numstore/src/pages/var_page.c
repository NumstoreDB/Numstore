...
#include "numstore/pages/var_page.h"
...
void vp_set_next (page *p, const pgno pg) { PAGE_SIMPLE_SET_IMPL (p, pg, VP_NEXT_OFST); }

void vp_set_ovnext (page *p, const pgno pg) { PAGE_SIMPLE_SET_IMPL (p, pg, VP_OVNX_OFST); }

void vp_set_vlen (page *p, const u16 vlen) { PAGE_SIMPLE_SET_IMPL (p, vlen, VP_VLEN_OFST); }

void vp_set_tlen (page *p, const u16 tlen) { PAGE_SIMPLE_SET_IMPL (p, tlen, VP_TLEN_OFST); }

void vp_set_root (page *p, const pgno root) { PAGE_SIMPLE_SET_IMPL (p, root, VP_ROOT_OFST); }

void vp_set_nbytes (page *p, const b_size nbytes) {
...
pgno vp_get_next (const page *p) { PAGE_SIMPLE_GET_IMPL (p, pgno, VP_NEXT_OFST); }

pgno vp_get_ovnext (const page *p) { PAGE_SIMPLE_GET_IMPL (p, pgno, VP_OVNX_OFST); }

u16 vp_get_vlen (const page *p) { PAGE_SIMPLE_GET_IMPL (p, u16, VP_VLEN_OFST); }

u16 vp_get_tlen (const page *p) { PAGE_SIMPLE_GET_IMPL (p, u16, VP_TLEN_OFST); }

pgno vp_get_root (const page *p) { PAGE_SIMPLE_GET_IMPL (p, pgno, VP_ROOT_OFST); }

b_size vp_get_nbytes (const page *p) { PAGE_SIMPLE_GET_IMPL (p, b_size, VP_NBYT_OFST); }
...
bool vp_is_overflow (const page *p) { return vp_calc_tofst (p) > PAGE_SIZE; }
...
void i_log_vp (const int level, const page *vp) {
