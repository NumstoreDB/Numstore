...
#ifndef NLOG
static const char *txn_state_to_str (const int state) {
  switch (state) {
...
static void i_log_txn_in_txnt (struct hnode *node, void *_log_level) {
...
static void merge_txn (struct txn *tx, void *vctx) {
...
static void find_max_undo (struct txn *tx, void *vctx) {
  slsn *max = vctx;
...
static void find_min_lsn (struct txn *tx, void *vctx) {
  slsn *min = vctx;
...
u32 txnt_get_size (const struct txn_table *dest) { return htable_size (dest->t); }

static bool txn_equals_for_exists (const struct hnode *left, const struct hnode *right) {
...
bool txn_exists (const struct txn_table *t, const txid tid) {
  struct txn tx;
  txn_key_init (&tx, tid);
...
bool txnt_get (struct txn **dest, struct txn_table *t, const txid tid) {
  DBG_ASSERT (txn_table, t);
...
void txnt_get_expect (struct txn **dest, struct txn_table *t, const txid tid) {
  DBG_ASSERT (txn_table, t);
...
void txnt_remove_txn_expect (struct txn_table *t, const struct txn *tx) {
  DBG_ASSERT (txn_table, t);
...
u32 txnt_serialize (u8 *dest, const u32 dlen, struct txn_table *t) {
  ASSERT (t->isfrozen);
...
struct txn_table *txnt_deserialize (const u8 *src, struct txn *txn_bank, const u32 slen, error *e) {
  struct txn_table *dest = txnt_open (e);
  if (dest == NULL) { goto failed; }
...
u32 txnlen_from_serialized (const u32 slen) {
  ASSERT (slen % TXN_SERIAL_UNIT == 0);
...
static void txnt_eq_foreach (struct hnode *node, void *_ctx) {
