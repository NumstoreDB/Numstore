...
// Setters
HEADER_FUNC void vt_set_next (page *p, const pgno pg) {
  PAGE_SIMPLE_SET_IMPL (p, pg, VT_NEXT_OFST);
}

// Getters
HEADER_FUNC pgno vt_get_next (const page *p) { PAGE_SIMPLE_GET_IMPL (p, pgno, VT_NEXT_OFST); }

HEADER_FUNC void vt_init_empty (page *p) {
  ASSERT (page_get_type (p) == PG_VAR_TAIL);
  vt_set_next (p, PGNO_NULL);
}

HEADER_FUNC struct bytes vt_get_bytes (page *p) {
  return (struct bytes){
...
HEADER_FUNC struct cbytes vt_get_bytes_imut (const page *p) {
