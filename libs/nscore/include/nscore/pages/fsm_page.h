...
HEADER_FUNC pgno pgtofsm (const pgno pg) {
  const pgno ret = pg / FS_BTMP_NPGS;
  return ret * FS_BTMP_NPGS;
}

HEADER_FUNC p_size pgtoidx (const pgno pg) { return pg % FS_BTMP_NPGS; }

////////////////////////////////////////////////////////////
/// GETTERS

HEADER_FUNC const void *fsm_get_bitmap_imut (const page *p) {
  return (void *)&p->raw[FS_BTMP_OFST];
}

HEADER_FUNC void *fsm_get_bitmap_mut (page *p) { return (void *)&p->raw[FS_BTMP_OFST]; }

HEADER_FUNC p_size fsm_get_bit (const page *p, const p_size idx) {
  return (((const u8 *)fsm_get_bitmap_imut (p))[idx / 8] >> (idx % 8)) & 1;
}

HEADER_FUNC sp_size fsm_next_freebit (const page *p, const p_size frombit) {
...
HEADER_FUNC void fsm_set_bit (page *p, const p_size idx) {
  ASSERT (idx < FS_BTMP_SIZE * 8);
  ((u8 *)fsm_get_bitmap_mut (p))[idx / 8] |= (1 << (idx % 8));
}

HEADER_FUNC void fsm_clr_bit (page *p, const p_size idx) {
