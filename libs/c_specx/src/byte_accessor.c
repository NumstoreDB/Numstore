...
static u32 ba_memcpy_from_recursive (u8 *dest, const u8 *src, struct byte_accessor *acc) {
  switch (acc->type) {
    case TA_TAKE: {
...
static u32 ba_memcpy_to_recursive (u8 *dest, const u8 *src, struct byte_accessor *acc) {
  switch (acc->type) {
    case TA_TAKE: {
      memcpy (dest, src, acc->src_size);
      return acc->src_size;
