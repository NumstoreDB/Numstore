...
static struct chunk *chunk_create (const u32 size, error *e) {
...
static u32 compute_new_chunk_size (const struct chunk_alloc *ca, const u32 alloc_size) {
  ...
  if (new_chunk_size < alloc_size) { new_chunk_size = alloc_size; }
...
static err_t chunk_alloc_add_new_chunk (struct chunk_alloc *ca, const u32 size, error *e) {
  DBG_ASSERT (chunk_alloc, ca);
  ...
  if (new_chunk == NULL) { return error_trace (e); }
...
void *chunk_malloc (struct chunk_alloc *ca, const u32 req, const u32 size, error *e) {
  ...
  if (ptr != NULL) { ca->total_used += alloc_size; }
...
void *chunk_calloc (struct chunk_alloc *ca, const u32 req, const u32 size, error *e) {
  DBG_ASSERT (chunk_alloc, ca);

  void *ptr = chunk_malloc (ca, req, size, e);
  if (ptr != NULL) { memset (ptr, 0, req * size); }
  return ptr;
}

void *chunk_alloc_move_mem (struct chunk_alloc *ca, const void *ptr, const u32 size, error *e) {
