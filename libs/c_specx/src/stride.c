...
bool ustride_equal (const struct user_stride left, const struct user_stride right) {
...
void stride_resolve_expect (struct stride *dest, const struct user_stride src, const u64 arrlen) {
...
err_t musb_accept_key (struct mus_builder *eb, const struct user_stride stride, error *e) {
...
err_t musb_build (struct multi_user_stride *dest, const struct mus_builder *eb, error *e) {
  DBG_ASSERT (mus_builder, eb);
  ASSERT (dest);
