...
const char *smfile_strerror (smfile_t *ns) { return nsh_strerror ((struct nshandle *)ns); }

int smfile_cleanup (const char *path) { return nsh_cleanup (path); }

// Core Operations
sb_size smfile_size (smfile_t *smf) { return smfile_psize (smf, NULL); }
...
smfile_t *smfile_new_context (smfile_t *n) {
  return (smfile_t *)nsh_new_context ((struct nshandle *)n);
}

int smfile_delete (smfile_t *n, const char *vname) {
  return nsh_delete ((struct nshandle *)n, vname);
}

int smfile_close (smfile_t *ns) { return nsh_close ((struct nshandle *)ns); }
int _smfile_crash (smfile_t *ns) { return nsh_crash ((struct nshandle *)ns); }

int smfile_begin (smfile_t *_smf) { return nsh_begin ((struct nshandle *)_smf); }
int smfile_commit (smfile_t *_smf) { return nsh_commit ((struct nshandle *)_smf); }
int smfile_rollback (smfile_t *smf) { return nsh_rollback ((struct nshandle *)smf); }
