...
/// and a polling state
pub struct PollingServer<L: Logger> {
...
impl PollingServer<ConsoleLogger> {
    pub fn new_default(addr: &str) -> Result<Self, Error> {
        PollingServer::new(addr, ConsoleLogger::new())
    }
}

impl<L: Logger> PollingServer<L> {
    /// Opens a new polling server on the specified address
    pub fn new(addr: &str, logger: L) -> Result<Self, Error> {
            ...
            match event.token() {
                ...
                token => {
                    ...
                    if event.is_read_closed() || event.is_write_closed() {
                        log_info!(self.logger, "Closed connection");
                    ...
                    log_info!(self.logger, "Executed server step");
                    log_info!(self.logger, "State before: {}", before);
                    log_info!(self.logger, "State after: {}", after);
