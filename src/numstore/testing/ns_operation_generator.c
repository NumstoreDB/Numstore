...
static char *
get_random_name (struct arena_alloc *alloc, error *e)
{
  u32   length = get_random_name_len ();
  char *buffer = arena_malloc (alloc, length + 1, 1, e);
  if (buffer == NULL) {
    return NULL;
  }
...
static char *
get_random_unique_name (struct ns_ref *ref, struct arena_alloc *alloc, error *e)
{
  ALLOC_INIT (temp);
  char *ret = NULL;
  ...
  while (true) {
    ...
    ret     = arena_malloc (alloc, len + 1, 1, e);
    if (ret == NULL) {
      goto theend;
    }
...
static char *
get_random_other_existing_name (struct ns_ref *ref, struct arena_alloc *alloc, error *e)
{
  ...
  while (true) {
    ...
    u32   len = strlen (cand);
    char *ret = arena_malloc (alloc, len + 1, 1, e);
    if (ret == NULL) {
      return NULL;
    }
...
static char *
get_random_other_existing_name_or_null (struct ns_ref *ref, struct arena_alloc *alloc, error *e)
{
