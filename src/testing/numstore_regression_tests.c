...
TEST (regression_cgd_test_create_delete_rollback_delete)
{
  ...
  test_assert_int_equal (
      nsdb_execute (db, "create n8Si3C union { tok6UW u32, YGhr cf128, LDzpWVm f16 }", NULL),
      0
  );
...
TEST (regression_cgd_test_create_crash_close_delete)
{
  ...
  // Create
  test_assert_int_equal (nsdb_execute (db, "create MkWMJ9a [8][9][3][3] i16", NULL), 0);
...
TEST (regression_irwr_rollback_invalid_wal_header)
{
  ...
  // TXN 4 (auto): INSERT ofst=0 nelem=53797
  {
    ...
    test_assert_int_equal (nsdb_execute (db, "insert testvar %d %d", data, 0, 53797), 53797);
    i_free (data);
  }
  ...
  // TXN 5 (auto): WRITE start=23070 stride=7888 stop=54622 nelems=4
  {
    ...
    test_assert_int_equal (nsdb_execute (db, "write testvar[23070:54622:7888]", data), 4);
  }
  ...
  {
    u32 removed[2];
    test_assert_int_equal (nsdb_execute (db, "remove testvar[5512:32808:13648]", removed), 2);
  }
  test_assert_int_equal (nsdb_commit (db), 0);
  ...
  // TXN 7 (auto): WRITE start=50236 stride=283 stop=51085 nelems=3
  {
    ...
    test_assert_int_equal (nsdb_execute (db, "write testvar[50236:51085:283]", data), 3);
  }
  ...
  {
    u32 removed[2];
    test_assert_int_equal (nsdb_execute (db, "remove testvar[51429:55291:1931]", removed), 2);
  }

  // TXN 10 (auto): READ start=1632 stride=9623 stop=20878 nelems=2
  {
    u32 buf[2];
    test_assert_int_equal (nsdb_execute (db, "read testvar[1632:20878:9623]", buf), 2);
  }

  // TXN 11 (auto): READ start=48723 stride=4036 stop=56795 nelems=2
  {
    u32 buf[2];
    test_assert_int_equal (nsdb_execute (db, "read testvar[48723:56795:4036]", buf), 2);
  }
  ...
  // WRITE start=49014 stride=3051 stop=52065 nelems=1
  {
    ...
    test_assert_int_equal (nsdb_execute (db, "write testvar[49014:52065:3051]", data), 1);
  }
  ...
  // INSERT ofst=22727 nelem=73857
  {
    ...
    test_assert_int_equal (nsdb_execute (db, "insert testvar %d %d", data, 22727, 73857), 73857);
    i_free (data);
  }
  ...
  {
    u32 removed[2];
    test_assert_int_equal (nsdb_execute (db, "remove testvar[5509:190235:92363]", removed), 2);
  }
  ...
  // INSERT ofst=8986 nelem=15959
  {
    ...
    test_assert_int_equal (nsdb_execute (db, "insert testvar %d %d", data, 8986, 15959), 15959);
    i_free (data);
  }
  ...
  {
    u32 buf[2];
    test_assert_int_equal (nsdb_execute (db, "read testvar[118059:145411:13676]", buf), 2);
  }
  ...
  // WRITE start=58530 stride=22447 stop=103424 nelems=2
  {
    ...
    test_assert_int_equal (nsdb_execute (db, "write testvar[58530:103424:22447]", data), 2);
  }
  ...
  // INSERT ofst=29193 nelem=27045
  {
    ...
    test_assert_int_equal (nsdb_execute (db, "insert testvar %d %d", data, 29193, 27045), 27045);
    i_free (data);
  }
  ...
  {
    u32 buf[1];
    test_assert_int_equal (nsdb_execute (db, "read testvar[39413:88949:49536]", buf), 1);
  }
