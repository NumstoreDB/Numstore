 ...
 * @brief Node Updates
 *
 * Node updates are used during the rebalance phase
 * It's a left and write list of key / value pairs for size and page location of
 * sub trees
 */
 ...
 * ----------------------------------------------------------------------------
 *
 * TODO - deeper technical explanation
 ******************************************************************************/

/**
 * @def Convenience
 * @brief Convenience macro: expands a page_h into its pgno and data-list size
 * for use in in_pair initializers
 */
#define pgh_unravel(pg) page_h_pgno (pg), dlgt_get_size (page_h_ro (pg))

/**
 * @brief Allocates and initializes a node_updates object with a given pivot
 * page
 *
...
/**
 * @brief Commits the first right-side update, replacing any existing
 * first-right entry
 *
 ...
 */
err_t
nupd_commit_1st_right (struct node_updates *s, pgno pg, b_size size, error *e);
 ...
 */
err_t
nupd_commit_1st_left (struct node_updates *s, pgno pg, b_size size, error *e);
 ...
 */
err_t
nupd_observe_pivot (struct node_updates *s, page_h *pg, p_size lidx, error *e);
...
/**
 * @brief Returns true if the left side has no remaining observations or pending
 * updates
 *
 * @param s The node_updates to query
 */
...
/**
 * @brief Returns true if the right side has no remaining observations or
 * pending updates
 *
 * @param s The node_updates to query
 */
...
/**
 * @brief Appends as many keys as possible from the left side of pg into the
 * left update list
 *
 * Reads up to rlen keys from the right end of pg, stopping when the list is
 * full.
 *
 ...
 * right update list
 *
 * Reads up to llen keys from the left end of pg, stopping when the list is
 * full.
 *
 * @param n    The node_updates to append into
 * @param pg   The inner-node page to read keys from
